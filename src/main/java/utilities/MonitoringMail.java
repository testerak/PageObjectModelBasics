...
public class MonitoringMail {
	...
	public void sendMail(String mailServer, String from, String[] to, String subject, String messageBody,
		...
		try {
			...
			// set up attachment to be sent
			BodyPart attachment = new MimeBodyPart();// Comment out if attachment not in email
			DataSource source = new FileDataSource(attachmentPath);// Comment out if attachment not in email
			attachment.setDataHandler(new DataHandler(source));// Comment out if attachment not in email
			attachment.setFileName(attachmentName);// Comment out if attachment not in email

			MimeMultipart multipart = new MimeMultipart();
			multipart.addBodyPart(body);

			// adds attachment to mail
			multipart.addBodyPart(attachment); // Comment out if attachment not in email
